...
def calculate_mae_sensitivity_table(plant_mw, virtual_mw, hub_stats, capture_rates, fee_rate):
    """Generate MAE sensitivity table with $/MW/Year for each MAE level"""
    mae_levels = [3.0, 4.0, 5.0, 6.0, 7.0, 8.0, 9.0, 10.0]
    results = []
    ...
    for m in mae_levels:
        ...
        rn_total = rv
        vt_total = va + vh
        combined_total = rv + va + vh
        
        results.append({
            'MAE ($/MWh)': f'${m:.0f}',
            'RN $/MW/Yr': rn_total / plant_mw,
            'VT $/MW/Yr': vt_total / plant_mw,
            'Combined $/MW/Yr': combined_total / plant_mw,
            'FG Rev $/MW/Yr': (combined_total / plant_mw) * (fee_rate / 100),
        })
...
with tab4:
    ...
    # MAE SENSITIVITY TABLE - NEW!
    st.markdown("### 📊 MAE Sensitivity Table ($/MW/Year)")
    st.caption("Shows how value per MW changes with forecast accuracy")
    
    mae_table = calculate_mae_sensitivity_table(plant_mw, virtual_mw, hub_stats, capture_rates, fee_rate)
    
    # Format the table
    mae_table_display = mae_table.copy()
    mae_table_display['RN $/MW/Yr'] = mae_table_display['RN $/MW/Yr'].apply(lambda x: f'${x:,.0f}')
    mae_table_display['VT $/MW/Yr'] = mae_table_display['VT $/MW/Yr'].apply(lambda x: f'${x:,.0f}')
    mae_table_display['Combined $/MW/Yr'] = mae_table_display['Combined $/MW/Yr'].apply(lambda x: f'${x:,.0f}')
    mae_table_display['FG Rev $/MW/Yr'] = mae_table_display['FG Rev $/MW/Yr'].apply(lambda x: f'${x:,.0f}')
    
    st.dataframe(mae_table_display, hide_index=True, use_container_width=True)
    
    st.caption(f"📍 Current MAE: ${mae:.1f}/MWh | Current Combined: ${value_per_mw:,.0f}/MW/Year")
    
    st.markdown("---")
    
    st.markdown("### 📈 MAE Sensitivity Chart")
    ...
    for m in mae_range:
        ...
        va = calculate_vt_alpha(virtual_mw, m, hub_stats, capture_rates) * BASE_DATA['annualization_factor']
        vh = calculate_vt_hedge(plant_mw, m, capture_rates) * BASE_DATA['annualization_factor']
        mae_values_rn.append(rv / plant_mw)  # Per MW
        mae_values_vt.append((va + vh) / plant_mw)
        mae_values_combined.append((rv + va + vh) / plant_mw)
...
with tab6:
    ...
    # MAE SENSITIVITY TABLE IN DATA SUMMARY - NEW!
    st.markdown("### 📊 MAE vs $/MW/Year Reference Table")
    st.caption("Quick reference: how forecast accuracy impacts value creation")
    
    mae_summary = calculate_mae_sensitivity_table(plant_mw, virtual_mw, hub_stats, capture_rates, fee_rate)
    mae_summary_display = mae_summary.copy()
    mae_summary_display['RN $/MW/Yr'] = mae_summary_display['RN $/MW/Yr'].apply(lambda x: f'${x:,.0f}')
    mae_summary_display['VT $/MW/Yr'] = mae_summary_display['VT $/MW/Yr'].apply(lambda x: f'${x:,.0f}')
    mae_summary_display['Combined $/MW/Yr'] = mae_summary_display['Combined $/MW/Yr'].apply(lambda x: f'${x:,.0f}')
    mae_summary_display['FG Rev $/MW/Yr'] = mae_summary_display['FG Rev $/MW/Yr'].apply(lambda x: f'${x:,.0f}')
    
    st.dataframe(mae_summary_display, hide_index=True, use_container_width=True)
    
    # Highlight current MAE
    st.info(f"📍 **Current Selection:** MAE = ${mae:.1f}/MWh → Combined = ${value_per_mw:,.0f}/MW/Year → FG Revenue = ${value_per_mw * fee_rate/100:,.0f}/MW/Year")
...
# ============================================
st.sidebar.markdown("---")
st.sidebar.markdown("*FG-GPT Value Creation Model v6.0*")
