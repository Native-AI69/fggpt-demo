...
def scale_for_capture_rates(base_value, zones, cap_high, cap_med, cap_low):
    # Base was calculated at 90/50/10
...
st.sidebar.markdown("---")
st.sidebar.markdown("*FG-GPT v8.0*")
...
with tab2:
    if scenario == "VT Only":
        ...
        # Calculate weighted capture
        weighted_cap = zones["high"] * (cap_high/100) + zones["med"] * (cap_med/100) + zones["low"] * (cap_low/100)
        ...
        zone_data = {
            ...
            "Weighted": ["{:.1f}%".format(zones["high"] * cap_high/100), "{:.1f}%".format(zones["med"] * cap_med/100), "{:.1f}%".format(zones["low"] * cap_low/100)],
        }
        st.dataframe(pd.DataFrame(zone_data), hide_index=True)
        st.markdown("**Effective Capture Rate:** {:.1f}%".format(weighted_cap))
