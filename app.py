...
VT_HEDGE_BASE = {3: 24357, 4: 23352, 5: 22329, 6: 21327, 7: 20411, 8: 19374, 9: 18417, 10: 17654, 11: 16915, 12: 16229}
...
if scenario == "RN Only":
    st.sidebar.caption("⚠️ Hub disabled - RN Only mode")
    selected_hub = st.sidebar.selectbox("Alpha Hub", list(HUB_STATS.keys()), index=0, disabled=True)
else:
    st.sidebar.caption("Affects VT Alpha only")
    selected_hub = st.sidebar.selectbox("Alpha Hub", list(HUB_STATS.keys()), index=0)

st.sidebar.markdown("---")
st.sidebar.markdown("*FG-GPT v9.0*")
...
vt_alpha_base = VT_ALPHA_BASE[selected_hub].get(mae, VT_ALPHA_BASE[selected_hub][6])
vt_alpha_per_mw = scale_for_capture_rates(vt_alpha_base, zones, cap_high, cap_med, cap_low)

vt_hedge_base = VT_HEDGE_BASE.get(mae, VT_HEDGE_BASE[6])
...
st.caption("7 Ranch Solar (" + str(plant_mw) + " MW) | Jan-Sep 2025 | " + scenario_label + " | MAE: $" + str(mae) + "/MWh" + (" | Hub: " + selected_hub if scenario != "RN Only" else ""))
...
with tab2:
    if scenario == "VT Only":
        ...
        # Calculate prorated uplift for each zone
        if total_weighted > 0:
...
with tab3:
    if scenario == "RN Only":
        ...
        # VT Zone Distribution Table (similar to RN)
        st.markdown("### VT Zone Distribution at MAE=$" + str(mae))
        
        vt_total = vt_alpha_annual + vt_hedge_annual
        weighted_high = zones["high"] * (cap_high/100)
        weighted_med = zones["med"] * (cap_med/100)
        weighted_low = zones["low"] * (cap_low/100)
        total_weighted = weighted_high + weighted_med + weighted_low
        ...
        if total_weighted > 0:
            uplift_high = vt_total * (weighted_high / total_weighted)
            uplift_med = vt_total * (weighted_med / total_weighted)
            uplift_low = vt_total * (weighted_low / total_weighted)
        else:
            uplift_high = uplift_med = uplift_low = 0
        ...
        vt_zone_data = {
            ...
            "Capture Rate": [str(cap_high) + "%", str(cap_med) + "%", str(cap_low) + "%", "-"],
            "Weighted": ["{:.1f}%".format(weighted_high), "{:.1f}%".format(weighted_med), "{:.1f}%".format(weighted_low), "{:.1f}%".format(total_weighted)],
            "Uplift ($)": ["${:,.0f}".format(uplift_high), "${:,.0f}".format(uplift_med), "${:,.0f}".format(uplift_low), "${:,.0f}".format(vt_total)],
            "FG Revenue ($)": ["${:,.0f}".format(fg_high), "${:,.0f}".format(fg_med), "${:,.0f}".format(fg_low), "${:,.0f}".format(vt_total * fee_rate / 100)],
        }
        st.dataframe(pd.DataFrame(vt_zone_data), hide_index=True, use_container_width=True)
        ...
        for hub in HUB_STATS.keys():
            ...
            hub_rows.append({
                "Hub": hub + (" ✓" if hub == selected_hub else ""),
...
with tab4:
    ...
    for m in range(3, 13):
        ...
        alpha_v = scale_for_capture_rates(VT_ALPHA_BASE[selected_hub][m], z, cap_high, cap_med, cap_low)
        hedge_v = scale_for_capture_rates(VT_HEDGE_BASE[m], z, cap_high, cap_med, cap_low)
    ...
    for m in mae_range:
        z = ZONE_DISTRIBUTION[m]
        rn_v = scale_for_capture_rates(RN_BASE[m], z, cap_high, cap_med, cap_low)
        alpha_v = scale_for_capture_rates(VT_ALPHA_BASE[selected_hub][m], z, cap_high, cap_med, cap_low) * virtual_mw / plant_mw if plant_mw > 0 else 0
        hedge_v = scale_for_capture_rates(VT_HEDGE_BASE[m], z, cap_high, cap_med, cap_low)
        rn_vals.append(rn_v)
...
with tab6:
    ...
    with c1:
        ...
        if scenario != "RN Only":
            params.append(["Hub", selected_hub])
    ...
    with c2:
        ...
        if scenario != "RN Only":
            results.append(["VT Alpha", "${:,.0f}".format(vt_alpha_display)])
            results.append(["VT Hedge", "${:,.0f}".format(vt_hedge_display)])
