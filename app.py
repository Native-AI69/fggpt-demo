...
with tab4:
    ...
    for m in range(3, 13):
        ...
        vt_a = VT_ALPHA_PER_MW[m] * virtual_mw / plant_mw if plant_mw > 0 else 0
    ...
    for m in mae_range:
        ...
        vt_h = VT_HEDGE_PER_MW[m]
        rn_vals.append(rn_v)
        vt_vals.append(vt_a + vt_h)
        combined_vals.append(rn_v + vt_a + vt_h)
    
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=mae_range, y=combined_vals, mode='lines+markers', name='Combined', line=dict(color='#9467bd')))
